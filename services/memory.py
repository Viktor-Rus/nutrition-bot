...
def build_user_memory_context(telegram_id: int):
    ...
    return {
        ...
        "content": (
            ...
            "Если в блюде, тексте или на фото есть продукт, который пользователь сохранил как 'не ем', "
            "'не переношу', 'аллергия' или другой запрет/ограничение, обязательно коротко отметь это "
            "как персональный конфликт и предложи замену. Не называй такой продукт плюсом блюда для "
            "этого пользователя и не советуй добавлять его или похожие нежелательные продукты. "

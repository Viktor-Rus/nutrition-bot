...
CALM_COMPANION_TONE_INSTRUCTION = (
    "Единый тон MealAdvisor: ты спокойный личный напарник по питанию, а не строгий проверяющий. "
    "Пиши на 'ты', тепло, уверенно и без морализаторства. Снижай тревожность: один приём пищи "
    "не ломает прогресс, важнее общий ритм и следующий спокойный выбор. "
    "Критикуй не пользователя, а конкретный состав еды или ситуацию. "
    "Не звучать как анкета, чек-лист или лекция. Если ответ простой, отвечай коротко. "
    "Если данных мало, не выдумывай детали: мягко задай один уточняющий вопрос или дай совет с оговоркой. "
    "Учитывай профиль, цель, предпочтения и ограничения пользователя, но упоминай их только когда это реально полезно. "
    "Оформляй ответы лёгкими блоками: эмодзи + короткий заголовок, затем 1-2 коротких предложения "
    "или пункты через символ •. Не используй Markdown-разметку: без **жирного**, ### и списков через дефис."
)
...
FOOD_ANALYSIS_FORMAT_INSTRUCTION = (
    ...
    "обычно используй 3-5 небольших блоков из вариантов: 🍽️ Что вижу, 🧭 Вывод, "
    "✅ Что уже хорошо, 🔧 Что можно улучшить, 👣 Маленький шаг, 💬 Спокойно. "
    ...
    "ответ на полный шаблон. Дай короткую живую оценку в 2-3 небольших блока: что это, личный вывод "
    "под цель пользователя и нужен ли маленький шаг. Можно писать прямо: 'Нормальный вариант, тут "
...
GENERAL_NUTRITION_ADVICE_INSTRUCTION = (
    ...
    "Если пользователь тревожится, сначала спокойно нормализуй ситуацию, а потом дай действие. "
    "Оформляй ответ визуально блочно: короткое вступление в 1 строку, затем 2-4 блока. "
    "Каждый блок начинай с эмодзи и короткого заголовка на отдельной строке, например "
    "'💧 Сначала', '🍎 Если хочется сладкого', '🥚 Чтобы не сорваться', '👣 Маленький шаг'. "
    "После заголовка дай 1-2 коротких предложения или пункты через символ •. "
    "Между блоками оставляй пустую строку. Не пиши длинные сплошные абзацы. "
    "Не повторяй одну и ту же структуру в каждом ответе; меняй названия блоков под ситуацию. "
    ...
    "и списки через дефис. Для списков используй символ •. "
    "Не считай калории и БЖУ, если пользователь прямо не просит. "
...
MEAL_FOLLOW_UP_INSTRUCTION = (
    ...
    "Оформляй это не сплошным текстом, а короткими визуальными блоками: эмодзи + короткий "
    "заголовок на отдельной строке, затем 1-2 коротких предложения или пункты через •. "
...
def normalize_good_meal_analysis(answer: str) -> str:
    ...
    answer = re.sub(
        r"((?:🔧 Как улучшить|🔧 Что можно улучшить)\n)(.*?)(\n👣 Маленький шаг)",
        (
            r"\1"
...
def food_action_prompt(action: str):
    if action == FOOD_ACTION_IMPROVE_TOMORROW:
        return (
            ...
            "Оформи ответ в 2-3 коротких блока и заверши одним маленьким действием."
        )
    ...
    if action == FOOD_ACTION_REPLACEMENT:
        return (
            ...
            "Объясняй не сухо, а как спокойный напарник: что выбрать и почему это сработает."
        )
        ...
        context_input = [
        ...
        ] + meal_thread + [
            {
                ...
                "content": (
                    ...
                    "Сохраняй ощущение непрерывного диалога: пользователь нажал кнопку, "
                    "потому что хочет практическое продолжение, а не новый разбор с нуля."
                ),
            },
            {
                "role": "user",
...
async def build_photo_input(photo: types.PhotoSize):
...
def photo_analysis_prompt(photo_count: int = 1):
    album_prefix = ""
    if photo_count > 1:
        album_prefix = (
            f"Пользователь прислал {photo_count} фото одним альбомом. "
            "Проанализируй их вместе как один продукт, состав, упаковку или один приём пищи. "
            "Не отвечай отдельно по каждому фото, а собери один общий вывод. "
            "Если одно фото показывает состав, а другое внешний вид продукта, используй оба. "
        )

    return (
        f"{album_prefix}"
        f"{CALM_COMPANION_TONE_INSTRUCTION} "
        "Проанализируй фото еды. "
        "Ответь коротко, тепло и практически: что видишь, дай честную оценку и личный вывод, "
        ...
        "Если пользователь прислал состав продукта, сначала коротко скажи, что это за продукт, "
        "затем оцени состав и дай вывод: можно спокойно, лучше редко, или лучше заменить. "
        "Не считай калории и БЖУ, если пользователь прямо не просит. "
...
async def analyze_food_photos(message: types.Message, photos):
    telegram_id = message.from_user.id

    try:
        photo_inputs = []
        for photo in photos:
            photo_inputs.append(await build_photo_input(photo))

        history = get_chat_history(telegram_id, limit=8)
        photo_count = len(photo_inputs)
        user_content = "[Фото еды]" if photo_count == 1 else f"[Фото еды: {photo_count} фото]"
        ...
        context_input = [
            ...
            },
            {
                ...
                "content": [
                    {
                        "type": "input_text",
                        "text": photo_analysis_prompt(photo_count)
                    },
                    *photo_inputs,
                ]
            }
...
async def analyze_food_photo(message: types.Message):
    await analyze_food_photos(message, [message.photo[-1]])


async def analyze_food_photo_album(message: types.Message, messages):
    photos = [
        item.photo[-1]
        for item in messages
        if item.photo
    ]
    await analyze_food_photos(message, photos)

...
def start_offer_text(subscription=None):
    ...
    if is_subscription_auto_renewing(subscription):
        ...
        return (
            ...
            "Помогаю разбирать питание без жёстких запретов: фото еды, составы продуктов, "
            "рецепты и простые шаги под твою цель.\n\n"
            "С чего начнём? Выбери кнопку ниже или просто пришли фото еды."
            f"{next_charge_note}"
        )
    ...
    if has_used_trial(subscription):
        return (
            ...
            f"Чтобы открыть анализ еды, рецепты и персональные рекомендации, подключи подписку: {format_amount()} в месяц. "
    ...
    return (
        ...
        "рецепты и простые шаги под твою цель.\n\n"
        "С чего начнём? Выбери кнопку ниже или просто пришли фото еды.\n\n"
        "Первая неделя бесплатно, карту привязывать не нужно.\n"
        f"После пробного периода подписка стоит {format_amount()} в месяц."
    )
